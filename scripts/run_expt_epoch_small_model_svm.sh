...
create_synth_files() { # $1:num_features $2:density
  ...
  train_file="/users/navsan/Programming/obamadb/data/_synth_svm_sm${num_features}_d${density}.train.3.tsv"
  test_file="/users/navsan/Programming/obamadb/data/_synth_svm_sm${num_features}_d${density}.test.tsv"
...
run_experiment_for_num_features() {
  num_features="$1"
  for density in 0.01 0.02 0.1 0.2; do
    create_synth_files $num_features $density
    for t in 1 10; do
    #for t in 1 2 4 6 8 10; do
      out_file="/users/navsan/Programming/obamadb/epoch_results/small_model_after_inlining/output_${i}_m${num_features}_d${density}_${t}.1.txt"
...
for i in `seq 1 2`; do
  for num_features in 1000.0 2000.0 4000.0; do

...
//        CHECK(sign(oldsum) * sign(dotsum) >= 0 || sign(prod) != sign(oldsum)) << "overflow @ addition";
    ...
    float fractionMisclassified(const fvector &theta, std::vector<SparseDataBlock<int_t> *> const & blocks) {
    ...
    float rmsError(const fvector &theta, std::vector<SparseDataBlock<int_t> *> const & blocks) {
    ...
    float rmsErrorLoss(const fvector &theta, std::vector<SparseDataBlock<int_t> *> const & blocks) {
      ...
      for (int i = 0; i < blocks.size(); i++) {
        ...
        for (int i = 0; i < block.getNumRows(); i++) {
          ...
          loss += std::max(kScaleFloats - dot_prod * classification, 0);
        }
        total_examples += block.getNumRows();
      }
      loss /= kScaleFloats;

...
namespace obamadb {
  ...
  namespace ml {
     ...
     * Dot product
     */
    num_t dot(const dvector<num_t> &v1, num_t *d2) {
      num_t sum = 0;
      num_t *__restrict__ pv1 = v1.values_;
      num_t *__restrict__ pv2 = d2;
    ...
    int numMisclassified(const fvector &theta, const SparseDataBlock<num_t> &block) {
      ...
      for (int i = 0; i < block.getNumRows(); i++) {
        ...
        const num_t dot_prod = dot(row, theta.values_);
    ...
    double fractionMisclassified(const fvector &theta, std::vector<SparseDataBlock<num_t> *> const & blocks) {
    ...
    double rmsError(const fvector &theta, std::vector<SparseDataBlock<num_t> *> const & blocks) {
